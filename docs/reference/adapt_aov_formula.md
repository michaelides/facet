...
warnings. This is only used by the aov estimator; lme4 and brms are
unaffected.

## Usage

``` r
adapt_aov_formula(formula, data, nested = NULL)
```
...
- Returns the rewritten aov formula string and a name mapping that
  [`compute_aov_vc_from_results()`](https://github.com/yourorg/facet/reference/compute_aov_vc_from_results.md)
