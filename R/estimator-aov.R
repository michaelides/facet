#' ANOVA-based Estimation Estimator for G-Studies
#'
#' Functions to fit G-study models using the method of moments (ANOVA-based
#' variance component estimation). This is an internal estimator used by [gstudy()].
#'
#' @name aov-estimator
#' @keywords internal
NULL
...
#'
#' @format A list with class "aovfit" containing:
#' \describe{
...
#' @return An object of class "aovfit" containing:
...
#'   item = factor(rep(1:5, times = 20))
#' )
#' fit_aov(score ~ (1 | person) + (1 | item), data)
#' }
#'
...
#' @export
#' @rdname fit_aov
fit_aov <- function(formula, data, unbalanced = FALSE, nested = NULL, ...) {
  # Check if formula is multivariate
  if (is_multivariate(formula)) {
    return(fit_aov_multivariate(formula, data,
  ...
  adapted <- adapt_aov_formula(formula, data, nested = nested)
  ...
  # rewrites from adapt_aov_formula() so the user-supplied component
  # names are preserved (e.g. "Rater" instead of "Rater:Task").
  vc <- compute_aov_vc_from_results(
    anova_results, data, response, random_facets, random_facet_specs,
    name_mapping = name_mapping
  )

  # Create the aovfit object
  result <- structure(
    list(
      formula = formula,
      data = data,
...
#' is only used by the aov estimator; lme4 and brms are unaffected.
#'
#' Behaviour:
...
#'   [compute_aov_vc_from_results()] applies to the resulting ANOVA table.
#'
...
#' @keywords internal
adapt_aov_formula <- function(formula, data, nested = NULL) {
  parsed <- parse_g_formula(formula)
  random_facets <- parsed$random_facets
  random_facet_specs <- parsed$random_facet_specs
...
#'   (e.g. "Rater"), as produced by [adapt_aov_formula()].
#'
#' @keywords internal
compute_aov_vc_from_results <- function(anova_results, data, response,
                                         random_facets,
  ...
  # Merge the user-overridden name mapping (from adapt_aov_formula) on
...
#' Fit Multivariate Model Using ANOVA-based Estimation
#'
...
#' @keywords internal
fit_aov_multivariate <- function(formula, data, unbalanced = FALSE, nested = NULL, ...) {
  if (unbalanced) {
    return(fit_aov_multivariate_unbalanced(formula, data, nested = nested, ...))
  }
  ...
  # Adapt aov Error() strata for nested facets (see adapt_aov_formula).
  ...
  if (compute_rescor || compute_re_cors) {
    correlations <- compute_aov_multivariate_correlations(
...
#' @param data A data frame containing the variables
#' @param ... Additional arguments
#' @return A aovfit object with unbalanced estimation
#' @keywords internal
fit_aov_multivariate_unbalanced <- function(formula, data, nested = NULL, ...) {
  # Parse the formula to get responses
  formula_char <- deparse(formula)
  ...
  if (compute_rescor || compute_re_cors) {
    correlations <- compute_aov_correlations_unbalanced(
...
#' @keywords internal
compute_aov_correlations_unbalanced <- function(data, responses, random_facets,
...
#' @param compute_re_cors Whether to compute random effect correlations
#' @keywords internal
compute_aov_multivariate_correlations <- function(data, responses, random_facets, aov_models, aov_formulas, random_effect_cors = list(),
...
#' Compute Variance Components from ANOVA Table using ANOVA-based Estimation
#'
#' @keywords internal
compute_aov_variance_components <- function(anova_table, design_info) {
...
#'
#' @export
#' @rdname extract_vc_aov
extract_vc_aov <- function(model, conf_level = 0.95, formula = NULL) {
  ...
  # The 'facet' column from compute_aov_vc_from_results is not needed
...
#'
#' @export
summary.aovfit <- function(object, ...) {
  cat("ANOVA-based Estimation Fit\n")
  cat("====================\n\n")
