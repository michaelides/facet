...
A significant advancement in Bayesian G-theory comes from the work of Duncan J.R. Jackson and colleagues, who demonstrated that traditional frequentist G-theory can confound multiple variance sources when facets are crossed (Jackson, Michaelides, Dewberry, & Kim, 2016).

Jackson et al.'s seminal work on assessment center ratings showed that when multiple facets are crossed (e.g., Person $\times$ Exercise $\times$ Rater designs), traditional ANOVA-based methods can conflate variance components, leading to misleading conclusions about the relative contribution of different sources. Their Bayesian unconfounding approach:

1. **Separates variance sources**: Uses Bayesian estimation to properly attribute variance to each source
2. **Quantifies uncertainty**: Provides full posterior distributions for variance components, enabling credible intervals for G-coefficients
3. **Handles small samples**: Performs better than frequentist methods with limited data

This work was extended to multisource performance ratings (Jackson, Michaelides, Dewberry, Schwencke, & Toms, 2020), demonstrating that unconfounding reveals different patterns of variance attribution in 360-degree feedback systems. Subsequent work examined uncertainty quantification (Jackson, Michaelides, Dewberry, & Kim, 2022) and provided comprehensive tutorials for G-theory application (Jackson, Michaelides, Stillman, & Atkins, 2023).

**LoPilato, Carter, & Wang (2015)** laid groundwork for Bayesian G-theory in management research, providing simulation evidence for prior selection and posterior summarization, demonstrating the advantages of Bayesian methods for variance component estimation.
...
## Jackson et al.'s Uncoupling Approach

### The Problem with Crossed Designs

Traditional G-theory can confound variance sources when facets are crossed. @jackson2016 demonstrated this problem in assessment center research:

**Person $\times$ Exercise $\times$ Rater design**: When raters are crossed with exercises (each rater rates each exercise), the traditional ANOVA decomposition conflates:

1. **True rater variance**: Differences in rater leniency/severity
2. **Exercise-specific rater variance**: Raters who are lenient on one exercise may be severe on another

This conflation leads to misinterpretation of variance components.

### The Uncoupling Solution

@jackson2016 showed that Bayesian estimation can "uncouple" these sources by:

1. **Modeling interactions explicitly**: Including Person $\times$ Rater, Person $\times$ Exercise, and Rater $\times$ Exercise interactions
2. **Separating variance components**: Each interaction has its own variance component
3. **Quantifying uncertainty**: Posterior distributions reveal which components are well-identified
...
### When to Use Uncoupling

Uncoupling is valuable when:

1. **Facets are fully crossed**: All combinations of facet levels are observed
2. **Interactions are theoretically meaningful**: You care about specific interactions
3. **Sample sizes are adequate**: More parameters require more data
4. **You need accurate attribution**: For diagnostic or validity research
...
- $Rel(S_i)$ is the reliability of subscale $i$ (G coefficient for relative, Phi for absolute)
- $\sigma_{S_i, S_j}$ is the observed covariance between subscales $i$ and $j$
- $Rel(C)$ is the reliability of the composite
- $\sigma^2_C$ is the observed variance of the composite

**Key insight**: When subscales are positively correlated, the covariance terms ($\sigma_{S_i, S_j}$) increase the numerator, making it more likely that the composite provides useful information for estimating the subscale's universe score. This is why VAR can be less than 1 even when subscale reliability is high—strong correlations with other subscales mean the composite "borrows strength" from these relationships.
